    ...
    private likePost(postId: number, $likeBtn: any, $likeCounter: any) {
        ...
        $.post(`${this.baseUrl}/likepost`, commonHelper.antiForgeryToken.add({
            ...
            if (data.totalLikes > 2)
                $likeCounter.html(`(you and ${data.totalLikes - 1} people)`);

            /*when the current user and only one person has liked the post*/
            else if (data.totalLikes -1 === 1)
                $likeCounter.html(`(you and ${data.totalLikes - 1} person)`);
